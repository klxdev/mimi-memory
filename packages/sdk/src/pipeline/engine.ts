...
export class PipelineEngine {
  ...
  async extractEntities(memory: Memory): Promise<Entity[]> {
    ...
    const extractionProviderName =
      Object.keys(this.config.providers).find((k) => k !== "local") ||
      embedderName;
    const extractionProvider = this.llmFactory.getProvider(
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        for (const re of rawEntities) {
          ...
          entity.vector = await embedder.embed(
            `${re.name}: ${re.type} - ${re.description}`,
          );
